...
namespace RESTApiWithAzureFunction
{
    public static class TaskListFunction
    {
            ...
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "task")] HttpRequest req, ILogger log)
        {
            ...
            var input = JsonConvert.DeserializeObject<Class1.CreateTaskModel>(requestBody);
            try
            {
        ...
        {
            List<Class1.TaskModel> taskList = new List<Class1.TaskModel>();
            try
            {
                using (SqlConnection connection = new SqlConnection(Environment.GetEnvironmentVariable("SqlConnectionString")))
                {
                    ...
                    while (reader.Read())
                    {
                        Class1.TaskModel task = new Class1.TaskModel()
                        {
                            Id = (int)reader["Id"],
        ...
        {
            string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
            var input = JsonConvert.DeserializeObject<Class1.UpdateTaskModel>(requestBody);
            try
            {
                using (SqlConnection connection = new SqlConnection(Environment.GetEnvironmentVariable("SqlConnectionString")))
                {
